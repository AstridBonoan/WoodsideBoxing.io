...
export function Footer() {
  return (
    <footer className="border-t border-white/10 bg-matte py-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="font-display text-2xl font-bold tracking-wider text-white uppercase">
              <span className="text-blood">{GYM.shortName}</span>
            </p>
            <p className="mt-2 max-w-xs text-sm text-white/50">
              {GYM.name} — Queens, NY. Real boxing. Real discipline.
            </p>
          </div>
          ...
          <div className="flex gap-4">
            {SOCIAL.map((s) => (
              <a
                ...
                href={s.href}
                aria-label={s.label}
                className="flex h-10 w-10 items-center justify-center rounded-sm border border-white/15 text-white/60 transition-colors hover:border-gold hover:text-gold"
              >
