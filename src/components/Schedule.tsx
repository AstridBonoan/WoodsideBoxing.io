...
export function Schedule() {
  ...
  return (
    <section id="schedule" ref={ref} className="animate-on-scroll border-t border-forest/20 bg-matte py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 hidden overflow-hidden rounded-sm border border-forest/30 md:block">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-forest/30 bg-forest-dark">
            ...
            <tbody>
              {SCHEDULE.map((row, i) => (
                <tr
                  key={row.type}
                  className={`border-b border-white/5 transition-colors hover:bg-forest/10 ${
                    i % 2 === 0 ? 'bg-matte/30' : ''
                  }`}
                >
        ...
        <div className="mt-10 space-y-4 md:hidden">
          {SCHEDULE.map((row) => (
            <div
              key={row.type}
              className="rounded-sm border border-forest/30 bg-forest-dark/40 p-5"
            >
